...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    ...
    .catch(err => {
      ...
      // window history API has methods that allow us to control the state of the browser's session. As long as this browser session has a preview page, we can go back there 
      window.history.back();
    });
...
function handleNewCommentSubmit(event) {
  ...
  // const searchPizzaParams = new URLSearchParams(document.location.search.substring(1));
  // const searchCommentParams = new URLSearchParams(document.location.search.substring(2));
  // const pizzaId = searchPizzaParams.get('id');
  // const commentId = searchCommentParams.get('id');
